    ...
    private static String getChar(int x, int y){
        ...
        for (BaseHero human: Main.allTeam) {
            if (human.getCoords()[0] == x && human.getCoords()[1] == y){
                ...
                if (Main.darkTeam.contains(human)) out = "|" + (AnsiColors.ANSI_GREEN + human.getInfo() + AnsiColors.ANSI_RESET);
                if (Main.holyTeam.contains(human)) out = "|" + (AnsiColors.ANSI_BLUE + human.getInfo() + AnsiColors.ANSI_RESET);
                break;
            }
        }
        return out;
